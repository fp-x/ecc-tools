...
describe 'ECC Tools', ->
  before ->
    @alicePrivateKey = Buffer.from('092cea22832ff99ad1d0bad976c7d354fbc938e0ba59711d7a66694c75291106', 'hex')
    @alicePublicKey = Buffer.from('03b471796adff784450ef8ed3018256bced96b8a770117fd7c6ce2fb6cf476b120', 'hex')
    @alicePublicKeyUncompressed = Buffer.from('04b471796adff784450ef8ed3018256bced96b8a770117fd7c6ce2fb6cf476b120db07652cb9a5b789e01df7fc1bbab98fa3d0880b1643df18742e5219d1684cb5', 'hex')
  ...
  describe 'Encoding/Decoding', ->
    describe 'xor', ->
      it 'should take two buffers and return the xor', ->
        result = ecc.xor(Buffer.from('0101', 'hex'), Buffer.from('0101', 'hex'))
        expect(result).to.eql(Buffer.from('0000', 'hex'))
        result = ecc.xor(Buffer.from('0100', 'hex'), Buffer.from('0101', 'hex'))
        expect(result).to.eql(Buffer.from('0001', 'hex'))

    describe 'zip', ->
      it 'should take two arrays of buffers and combine the buffer pairs', ->
        chunks1 = ecc.chunk(Buffer.from('EC00B99365A09B6E26C732378CD0C6257F7012BC', 'hex'), 5)
        chunks2 = ecc.chunk(Buffer.from('9365A09B6E26C732378CD0C6257F7012BC', 'hex'), 5)
        result = ecc.zip(chunks1, chunks2)
        expect(result[0]).to.eql(Buffer.from('EC00B993659365A09B6E', 'hex'))
        expect(result[1]).to.eql(Buffer.from('A09B6E26C726C732378C', 'hex'))
        expect(result[2]).to.eql(Buffer.from('32378CD0C6D0C6257F70', 'hex'))
        expect(result[3]).to.eql(Buffer.from('257F7012BC12BC', 'hex'))

    describe 'chunk', ->
      it 'should take a buffer and break it into chunks', ->
        result = ecc.chunk(Buffer.from('EC00B99365A09B6E26C732378CD0C6257F7012BC', 'hex'), 5)
        expect(result[0]).to.eql(Buffer.from('EC00B99365', 'hex'))
        expect(result[1]).to.eql(Buffer.from('A09B6E26C7', 'hex'))
        expect(result[2]).to.eql(Buffer.from('32378CD0C6', 'hex'))
        expect(result[3]).to.eql(Buffer.from('257F7012BC', 'hex'))

      it 'should leave the remainder in the last chunk', ->
        result = ecc.chunk(Buffer.from('EC00B99365A09B6E26C732378CD0C6257F7012BC', 'hex'), 6)
        expect(result[3]).to.eql(Buffer.from('12bc', 'hex'))

    describe 'combine', ->
      it 'should take two buffers and return the combination', ->
        result = ecc.combine(Buffer.from('EC00B99365A09B6E26C732378CD0C6257F7012BC', 'hex'), Buffer.from('00AFEA21', 'hex'))
        expect(result).to.eql(Buffer.from('00EC00B99365AFA09B6E26C7EA32378CD0C621257F7012BC', 'hex'))
    ...
    describe 'encode', ->
      it 'should return a string if passed a buffer', ->
        result = ecc.encode(Buffer.from('deadbeef','hex'))
        expect(typeof result).to.equal('string')

      it 'should return a buffer if passed a Base58 encoded string', ->
        result = ecc.encode(Buffer.from('00EC00B99365AFA09B6E26C7EA32378CD0C621257F7012BC','hex'), Buffer.from('953ABC69', 'hex'))
    ...
    describe 'decode', ->
      ...
      it 'should return a buffer if passed a hex encoded string that looks like a base58 encoded string', ->
        ...
        expect(result).to.be.an.instanceof(Buffer)
        expect(result).to.eql(Buffer.from('ffffffffffffffff', 'hex'))

    describe 'address', ->
      it 'should return an encoded address', ->
        key = Buffer.from('0284E5235E299AF81EBE1653AC5F06B60E13A3A81F918018CBD10CE695095B3E24','hex')
        version = Buffer.from('00AFEA21','hex')
        mask = Buffer.from('953ABC69','hex')
  ...
  describe 'ECDSA', ->
    describe 'sign', ->
      ...
      it 'should error if message is over 32bytes', ->
        signing = => ecc.sign(Buffer.alloc(33), @alicePrivateKey)
